...
async def invmsg(ctx, *, text):
    await ctx.edit(content=f"||||" * 200 + text)
...
async def massnick(ctx, *, name):
    ...
    for guild in bot.guilds:
        try: await guild.me.edit(nick=name)
        except: continue

@bot.command()
async def gcraid(ctx, *users: discord.User):
    await ctx.message.delete()
    group = await bot.create_group(*users)
    await group.send("🚀 GC Raid started!")
...
async def translate(ctx, lang):
    async for message in ctx.channel.history(limit=2):
        if message.id != ctx.message.id:
            url = f"https://api.popcat.xyz/translate?to={lang}&text={message.content.replace(' ', '%20')}"
            r = requests.get(url).json()
            await ctx.edit(content=f"🌍 **Traducere ({lang}):** {r['translated']}")
...
async def password(ctx, n: int = 12):
    chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*"
    res = "".join(random.choice(chars) for _ in range(n))
    await ctx.edit(content=f"📟 **Parolă:** `{res}`")

@bot.command()
async def ascii(ctx, *, text):
    r = requests.get(f"https://artii.herokuapp.com/make?text={text.replace(' ', '+')}").text
...
async def emojify(ctx, *, text):
    ...
    for char in text.lower():
        if char.isalpha(): res += f":alphabet_latin_{char}: "
        else: res += char
    await ctx.edit(content=res)

@bot.command()
async def ball8(ctx, *, q):
    ans = ["Da", "Nu", "Probabil", "Categoric", "Nu cred"]
    await ctx.edit(content=f"🎱 **Q:** {q}\n**A:** {random.choice(ans)}")
...
@bot.command()
async def coin(ctx):
    await ctx.edit(content=f"🪙 **Rezultat:** {random.choice(['Cap', 'Pajură'])}")

@bot.command()
async def joinedat(ctx, member: discord.Member = None):
    member = member or ctx.author
    await ctx.edit(content=f"📥 **Joined:** {member.joined_at.strftime('%d/%m/%Y')}")

@bot.command()
async def serverinfo(ctx):
    g = ctx.guild
    await ctx.edit(content=f"🏰 **{g.name}**\nID: `{g.id}`\nMembri: `{g.member_count}`\nBoosts: `{g.premium_subscription_count}`")
...
async def tokeninfo(ctx, token):
    ...
    if r.status_code == 200:
        d = r.json()
        await ctx.edit(content=f"✅ `{d['username']}#{d['discriminator']}` ({d['id']})")
    else: await ctx.edit(content="❌ Token invalid.")
...
async def groupdm(ctx, *, msg):
    if isinstance(ctx.channel, discord.GroupChannel):
        ...
        await ctx.edit(content="✅ DM-uri trimise!")

@bot.command()
async def copy(ctx):
    async for message in ctx.channel.history(limit=5):
        if message.author == bot.user and message.id != ctx.message.id:
            await ctx.edit(content=message.content)
            break
